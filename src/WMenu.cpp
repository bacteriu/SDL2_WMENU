 ...
 *
 * TODO:  Modificar todas las funciones que pueden establecer el campo Data para que el tic se
 *        se indique con el bit mas alto los demas bit se pueden usar para hacer grupos excluyentes.
 *        haciendo esto los datos para items tipo togle serian los 31 bit bajos.
 *
 * TODO:  Hacer la versión que utilice texturas en lugar de surfaces
 *
 * TODO:  Añadir puntero a la instancia que ha generado un item CALLBACK de forma que en la rutina de
 *          CALLBACK podamos saber de que instancia es el item picado.
 *
 ...
 * Esta función solo busca en item no en la barra de menú.
 * Parametros:
 *            item: El item que al que cambiar el campo data.
 *            data: El nuevo valor que se asignará al campo data
 * Retorna:   True si el item se encontro y cambio el campo data
 *            False si el item no se ha encontrado.
 ********************************************************************/
bool WMenu::setData(int32_t item, int data) {
    Menu_t* ptr=itemsearch(firstmenu,item);
    bool pret=false;
    if (ptr!=NULL) {
        ptr->data=data;
...
/********************************************************************
 * Obtiene el valor del campo data de un item, si el item es de tipo
 * WM_TOGLE entonces aqui estará el valor del TOGLE.
 * Esta función solo busca en item, no en la barra de menú.
 * Parametros:
 *            item: El item del que obtener el campo data.
 *            data: Una variable donde se almacenará el valor del
 *                  campo data.
 * Retorna: True si el item se encontró, entonces data es valida.
 *          False si el item no se encontro y data no contiene el
 *          valor de campo data.
 ********************************************************************/
bool WMenu::getData(int32_t item, int &data) {
    Menu_t* ptr=itemsearch(firstmenu, item, WM_DEFAULT);
    bool pret=false;
    if (ptr!=NULL) {
        data=ptr->data;
        pret=true;
...
/********************************************************************
 + Copia el titulo de un item en un buffer con maxima longitud leng
 * Parametros:
 *             item: El numero del item del que obtendremos el titulo
 *             buftitle: Un buffer donde copiar el titulo
 *             leng: Maxima longitud a copiar excluyendo el NULL
 * Retorna:    cantidad de caracteres copiados al buffer  ó
 *             0 si item no fué encontrado, leng=0 or buffer=NULL
 *
 ********************************************************************/
int32_t WMenu::getItemTitle(int32_t item, char* buftitle, int32_t leng) {
    Menu_t* ptr=itemsearch(firstmenu,item);
    int32_t cnt=0;
    if (buftitle !=NULL && ptr!=NULL && leng>0) {
        while (cnt<leng && *(ptr->mtitle+cnt)!=0 ) {
            *(buftitle+cnt)=*(ptr->mtitle+cnt);
            cnt++;
        }
        *(buftitle+cnt)=0;
    }
...
/********************************************************************
 * Retorna el tipo de un item para determinar si es WM_DEFAULT,
 * WM_TOGLE o WM_IMAGE.
 * Parametros:
 *              item: El item del que obtener su tipo
 * Retorna:     0 si el item esta en la barra de menu,
 *              1 si el item es de tipo WM_DEFAULT
 *              2 si el item es de tipo WM_TOGLE
 *              4 si el item es de tipo WM_IMAGE
 *******************************************************************/
int32_t WMenu::getItemType(int32_t item) {
    Menu_t* ptr=itemsearch(firstmenu,item);
    return (ptr!=NULL)? ptr->type:-1;
}
/********************************************************************
...
/********************************************************************
 * Esta funcion es llamada por drawMenu para dibujar la barra    *
